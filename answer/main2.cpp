...
auto dict_hash_to_candidate = MinimumHashMap<int, 1 << hash_table_size>();
...
struct State {
	...
	inline void GetNextStates(Node* const& parent_node) const {
		...
		using namespace beam_search;
		auto f = [&](State& new_state, const u8& p_remove, const u8& p_add) {
			auto& idx_old_node = dict_hash_to_candidate[new_state.hash];
			if (idx_old_node == -1) {
				// まだそのハッシュの状態が無い
				idx_old_node = q.size();
				q.push({ new_state.score, parent_node, nullptr, {p_remove, p_add} });
			}
			else if (q[idx_old_node].score < new_state.score) {
				// 同じハッシュでスコアがより良い
				q[idx_old_node].score = new_state.score;
				q[idx_old_node].parent_node = parent_node;
				q[idx_old_node].action = { p_remove, p_add };
			}
				...
				const auto rr = r.Right();
				{
					...
					const auto cond_center = d & ~u;
					{
						...
						for (const auto& p_remove : (cond_dl& machines & ~dl).NonzeroIndices()) {
							...
							ASSERT(p_remove != p_add, "元と同じ箇所は選ばれないはずだよ");  auto new_state = *this;  new_state.Do(Action{ p_remove, p_add });  f(new_state, p_remove, p_add);
						}
						// 右上から左下
...
Stack<State::Node, 500000> State::q;
...
void Solve() {
	...
	// ビームサーチ
	{
		...
		int beam_width;
		rep(t, T) {
			beam_search::dict_hash_to_candidate.clear();
			...
			for (auto parent_node = parent_nodes_begin; parent_node != parent_nodes_end; parent_node++) {
				...
				// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				//next_states.clear();
				parent_node->state->GetNextStates(parent_node);
				/*
