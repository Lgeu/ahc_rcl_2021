		...
		if (((int)n_machines + 1) * ((int)n_machines + 1) * ((int)n_machines + 1) > money || turn >= PURCHASE_TURN_LIMIT) {
			...
			if (n_machines == 1) {
				...
				// 特殊ケース 2: [ -> ]
				{
					...
					const auto cond_ud3 = l & r & dl & dr & ddl & ddr & dddl & dddr & ~u & ~dddd;
					// 上から下
					...
					for (const auto& p_add : (cond_ud3 & ~machines & ddd).NonzeroIndices()) {
						const auto p_remove = UpOf(UpOf(UpOf(p_add)));
						ASSERT(p_remove != p_add, "元と同じ箇所は選ばれないはずだよ");  auto new_state = *this;  new_state.Do(Action{ p_remove, p_add });  res.push(NewStateInfo{ new_state.score, new_state.hash, {p_remove, p_add} });
					}
					// 下から上
					...
					for (const auto& p_remove : (cond_ud3 & machines & ~ddd).NonzeroIndices()) {
						const auto p_add = UpOf(UpOf(UpOf(p_remove)));
						ASSERT(p_remove != p_add, "元と同じ箇所は選ばれないはずだよ");  auto new_state = *this;  new_state.Do(Action{ p_remove, p_add });  res.push(NewStateInfo{ new_state.score, new_state.hash, {p_remove, p_add} });
					}

					const auto cond_lr  = u & d & ur & dr                           & ~l & ~rr;
					const auto cond_lr2 = u & d & ur & dr & urr & drr               & ~l & ~rrr;
					const auto cond_lr3 = u & d & ur & dr & urr & drr & urrr & drrr & ~l & ~rrrr;
					// 左から右
					...
					for (const auto& p_add : (cond_lr3 & ~machines & rrr).NonzeroIndices()) {
						const auto p_remove = LeftOf(LeftOf(LeftOf(p_add)));
						ASSERT(p_remove != p_add, "元と同じ箇所は選ばれないはずだよ");  auto new_state = *this;  new_state.Do(Action{ p_remove, p_add });  res.push(NewStateInfo{ new_state.score, new_state.hash, {p_remove, p_add} });
					}
					// 右から左
					...
					for (const auto& p_remove : (cond_lr3 & machines & ~rrr).NonzeroIndices()) {
						const auto p_add = LeftOf(LeftOf(LeftOf(p_remove)));
						ASSERT(p_remove != p_add, "元と同じ箇所は選ばれないはずだよ");  auto new_state = *this;  new_state.Do(Action{ p_remove, p_add });  res.push(NewStateInfo{ new_state.score, new_state.hash, {p_remove, p_add} });
					}
